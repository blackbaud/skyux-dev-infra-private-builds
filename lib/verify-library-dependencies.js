    ...
    for (const packageName in projectPackageJson[section]) {
        ...
        if (workspaceVersion !== minWorkspaceVersion) {
            errors.push(`The version listed in the workspace 'package.json' for "${packageName}@${workspaceVersion}" must be set to a specific version ` +
                ...
                `section of '${projectRoot}/package.json' (wanted "${targetVersion}"). To address this problem, set ` +
                `"${packageName}" to (${minTargetVersion}) in the workspace 'package.json'.`);
            continue;
        }
        // Check for multiple versions.
        let workspaceSatisfiesTargetRange = false;
        const ranges = targetVersion.split('||').map((p) => p.trim());
        for (const range of ranges) {
            const version = semver_1.default.minVersion(range).version;
            if (version === minWorkspaceVersion) {
                workspaceSatisfiesTargetRange = true;
            }
        }
        if (!workspaceSatisfiesTargetRange) {
