...
async function publishPackagesDist(options) {
    ...
    if (!options.skipRegistryConfig && !npmToken) {
    ...
    const offsetVersions = Object.values((0, offset_versions_1.getOffsetVersions)(options.offsetPackageVersions ?? {}, version));
    ...
    if (!options.firstRun && !versions?.length) {
    ...
    const deprecateCommandArgs = ['deprecate'];
    if (options.dryRun) {
        commandArgs.push('--dry-run');
        deprecateCommandArgs.push('--dry-run');
    }
    const distPackages = await (0, get_publishable_projects_1.getPublishableProjects)();
    const knownGoodNamespaces = [];
    async function checkRegistryUrl(packageName, distRoot) {
        const namespace = packageName.startsWith('@')
            ? `${packageName.split('/').shift()}:`
            : '';
        if (knownGoodNamespaces.includes(namespace)) {
            return;
        }
        const registryUrl = await (0, npm_1.getPublishRegistry)(distRoot);
        if (registryUrl !== options.expectedRegistryUrl) {
            throw new Error(`The NPM registry is not configured for '${namespace}' (expected '${options.expectedRegistryUrl}', got '${registryUrl}').`);
        }
        knownGoodNamespaces.push(namespace);
    }
    const errors = [];
    if (!options.skipRegistryConfig && npmToken) {
        await createNpmrcFile(npmToken);
    }
    // First loop through, validating that each package can be published.
    for (const projectName in distPackages) {
        ...
        if (!(0, fs_extra_1.existsSync)((0, path_1.join)(distRoot, 'package.json'))) {
            throw new Error(`Path '${(0, path_1.join)(distRoot, 'package.json')}' does not exist. Did you run 'skyux-dev create-packages-dist'?`);
        }
        const { name: packageName, version: packageVersion } = await (0, fs_extra_1.readJson)((0, path_1.join)(distRoot, 'package.json'));
        if (version !== packageVersion &&
            !offsetVersions.includes(packageVersion)) {
            throw new Error(`Path '${distRoot}/package.json' has version '${packageVersion}' but this release is version '${version}'${offsetVersions.length
                ? ` (or ${offsetVersions.map((v) => `'${v}'`).join(', ')})`
                : ''}. Did you run 'skyux-dev create-packages-dist'?`);
        }
        await checkRegistryUrl(packageName, distRoot);
    }
    console.log(`\n\n ✅ Validated all package builds and registry configurations.\n\n`);
    // Second loop through, actually publishing each package.
    for (const projectName in distPackages) {
        const distRoot = (0, path_1.join)(CWD, distPackages[projectName].distRoot);
        const { name: packageName, version: packageVersion } = await (0, fs_extra_1.readJson)((0, path_1.join)(distRoot, 'package.json'));
        try {
...
`);
            ...
            const deprecationMessage = distPackages[projectName].deprecated;
            if (deprecationMessage) {
                console.log(`Deprecating package '${packageName}@${packageVersion}' with message: "${deprecationMessage}"`);
                await (0, spawn_1.runCommand)('npm', [
                    ...deprecateCommandArgs,
                    ...
                    throw new Error(`Error when deprecating '${projectName}'!`, {
                        cause: err,
                    });
                });
            }
