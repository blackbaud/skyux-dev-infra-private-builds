...
async function publishPackagesDist(options) {
    ...
    for (const projectName in distPackages) {
        ...
        node_assert_1.default.ok(typeof packageName === 'string');
        node_assert_1.default.ok(typeof packageVersion === 'string');
        ...
        distPackagesToPublish.push({ projectName, packageName, packageVersion });
    }
    console.log(`\n\n ✅ Validated all package builds and registry configurations.\n\n`);
    // Second loop through, actually publishing each package.
    for (const { projectName, packageName, packageVersion, } of distPackagesToPublish) {
        const distRoot = (0, path_1.join)(CWD, distPackages[projectName].distRoot);
        try {
            console.log(`
