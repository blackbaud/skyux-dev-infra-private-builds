...
async function buildI18nLibrary() {
    ...
    await (0, spawn_1.runCommand)('npx', ['nx', 'build', 'i18n'], { env: process.env });
...
async function createLibraryResources(argv) {
    ...
    const { resourcesModulePaths } = await (0, read_skyuxdev_config_1.readSkyuxdevConfig)(['resourcesModulePaths'], true);
    ...
    for (const projectName of projectNames) {
        ...
        const resourcesFiles = glob_1.glob.sync(`${project.root}/src/assets/locales/resources_*.json`);
        if (resourcesFiles.length === 0) {
            console.log(` [!] Resource files not found for project '${projectName}'. Skipping.`);
            continue;
        }
        let isEmptyOrInvalid;
        try {
        ...
        const i18nPath = isI18nLocalProject
            ...
            : '@skyux/i18n';
        const resourcesModulePath = resourcesModulePaths?.[projectName] ||
