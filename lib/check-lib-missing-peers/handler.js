...
exports.checkLibraryMissingPeers = void 0;
...
async function findUnlistedPeers(projectName, packageConfig, monorepoPackageNames, packageJson, fix = false) {
    const errors = [];
    ...
    for (const fileName of files) {
        ...
        const sourceFile = typescript_1.default.createSourceFile(fileName, contents, typescript_1.default.ScriptTarget.Latest, true);
        ...
        const dynamicImports = sourceFile.statements
            .filter((statement) => typescript_1.default.isImportTypeNode(statement))
            .map((statement) => statement.argument
            .getText()
            .replace(/['"]/g, ''))
            .filter((statement) => !statement.startsWith('.'));
        const foundPackages = new Set([...importedModules, ...dynamicImports]);
...
async function findUnusedPeers(projectName, packageConfig, packageJson, packagesFoundInSourceCode, fix = false) {
    const errors = [];
...
async function checkLibraryMissingPeers(options) {
    ...
    for (const projectName in packageConfigs) {
        ...
        const unlistedPeersResult = await findUnlistedPeers(projectName, packageConfig, monorepoPackageNames, packageJson, options.fix);
        const unusedPeersResult = await findUnusedPeers(projectName, packageConfig, packageJson, unlistedPeersResult.packagesFoundInSourceCode, options.fix);
        errors = errors
