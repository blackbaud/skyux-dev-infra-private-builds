...
function findUnusedPeers(projectName, packageConfig, packageJson, packagesFoundInSourceCode, fix = false, implicitPeers) {
    const errors = [];
...
async function checkLibraryMissingPeers(options) {
    ...
    for (const projectName in packageConfigs) {
        ...
        const unusedPeersResult = findUnusedPeers(projectName, packageConfig, packageJson, unlistedPeersResult.packagesFoundInSourceCode, options.fix, implicitPeers);
        errors = errors
