...
exports.githubPagesWait = exports.addOrUpdateGithubPrComment = exports.checkGithubPagesPublished = exports.getOpenPrNumbers = exports.getBranchBuildStatus = void 0;
...
async function checkGithubPagesPublished(pagesPath, commit, config) {
    ...
    return result.data.some((build) => build.commit.sha === commit);
}
...
async function githubPagesWait(workingDirectory, config) {
    ...
    for (let i = 1; i <= 10; i++) {
        ...
        await commandRunner('sleep', [`${i * 2}`]);
        if (await checkGithubPagesPublished(options.ownerSlashRepo, latestCommit, {
            token: options.token,
            ...
            // After 5 checks, push another commit to the repo to trigger a build.
            await commitCommand({
                workingDirectory,
    ...
    // After 10 checks, 110 seconds of sleep plus call times (so ~2 minutes), give up.
    if (!confirmed) {
