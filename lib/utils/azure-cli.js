"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.azLogin = azLogin;
exports.createOrUpdatePullRequest = createOrUpdatePullRequest;
const git_1 = require("./git");
const spawn_1 = require("./spawn");
async function azLogin(config) {
    if (!process.env['AZURE_DEVOPS_EXT_PAT']) {
        await (0, spawn_1.runCommand)('az', ['account', 'list-locations', '-o', 'none'], {}, {
            logCommand: !!config?.logCommand,
        }).catch(async () => {
            await (0, spawn_1.runCommand)('az', ['login'], {}, {
                logCommand: !!config?.logCommand,
            });
        });
    }
}
async function createOrUpdatePullRequest(sourceBranch, targetBranch, title, changelogEntry, config) {
    const options = {
        workingDirectory: process.cwd(),
        logCommand: false,
        ...config,
    };
    const repoUrlString = await (0, git_1.getRemoteUrl)({
        workingDirectory: options.workingDirectory,
        logCommand: options.logCommand,
    });
    const prJson = JSON.parse(await (0, spawn_1.getCommandOutput)(`az`, [
        `repos`,
        `pr`,
        `list`,
        `-t`,
        targetBranch,
        `-s`,
        sourceBranch,
        `-o`,
        `json`,
    ], {
        cwd: options.workingDirectory,
        stdio: 'pipe',
    }, {
        logCommand: options.logCommand,
    }).catch((err) => {
        console.error(`Error checking for PRs: ${err}`);
        throw err;
    }));
    // Description parameter accepts multiple arguments, one per line, and we prefix with spaces to avoid hyphens being misinterpreted.
    const descriptionLines = changelogEntry
        .trim()
        .split('\n')
        .map((line) => `  ${line}`);
    let prLink;
    if (prJson.length === 0) {
        const newPrJson = await (0, spawn_1.getCommandOutput)(`az`, [
            `repos`,
            `pr`,
            `create`,
            `--source-branch`,
            sourceBranch,
            `--target-branch`,
            targetBranch,
            `--title`,
            title,
            `--description`,
            ...descriptionLines,
            `-o`,
            `json`,
        ], {
            cwd: options.workingDirectory,
            stdio: 'pipe',
        }, {
            logCommand: options.logCommand,
        }).catch((err) => {
            console.error(`Error creating PR: ${err}`);
            throw err;
        });
        prLink = `${repoUrlString}/pullrequest/${JSON.parse(newPrJson)['pullRequestId']}`;
    }
    else {
        prLink = `${repoUrlString}/pullrequest/${prJson[0]['pullRequestId']}`;
        await (0, spawn_1.getCommandOutput)(`az`, [
            `repos`,
            `pr`,
            `update`,
            `--title`,
            title,
            `--description`,
            ...descriptionLines,
            `--id`,
            prJson[0]['pullRequestId'],
        ], {
            cwd: options.workingDirectory,
            stdio: 'ignore',
        }, {
            logCommand: options.logCommand,
        }).catch((err) => {
            console.error(`Error updating PR: ${err}`);
            throw err;
        });
    }
    const prLinkUrl = new URL(prLink);
    prLinkUrl.username = '';
    prLinkUrl.password = '';
    return { prLink: prLinkUrl.toString() };
}
