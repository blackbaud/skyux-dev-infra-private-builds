...
async function getE2eWorkflow(options) {
    ...
    const e2eProjects = await (0, get_project_names_1.getProjectNames)('e2e', 'all', all ? 'all' : 'affected', {
        getCommandOutput: runner,
    });
    ...
    const e2eTargets = e2eProjects.map((e2eProject) => {
        return {
            project: e2eProject,
    ...
    // Backwards-compatible filtered list of e2e projects, removing the -e2e suffix.
    const e2eMap = e2eTargets
        .filter((e2eTarget) => e2eTarget.project.match(/-e2e$/)) // Workflow assumes e2eMap projects end in -e2e.
        .map((e2eTarget) => {
        return {
            project: e2eTarget.project.replace(/-e2e$/, ''),
            token: e2eTarget.token,
            root: e2eTarget.root,
        };
    });
