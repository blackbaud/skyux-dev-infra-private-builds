...
async function getE2eWorkflow(options) {
    ...
    // Branch previews include all Storybooks; PR previews include only affected Storybooks; merge group does not include Storybooks.
    const projects = options.workflowTrigger === 'merge_group'
        ? []
        : await (0, get_project_names_1.getProjectNames)('build-storybook', 'all', all ? 'all' : 'affected', {
    ...
    const e2eTargets = e2eProjects.length === 0 || options.skipE2e
