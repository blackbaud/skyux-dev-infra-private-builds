...
jest.mock('fs-extra', () => ({
    ...
    existsSync: jest.fn().mockReturnValueOnce(true).mockReturnValue(false),
    removeSync: jest.fn(),
}));
describe('e2e-workflow', () => {
    ...
    it('should get e2e workflow for pr', async () => {
        ...
        const getCommandOutput = jest
            ...
            if (args.includes('--target=build-storybook')) {
                return Promise.resolve('project1, project2');
            }
            else if (args.includes('--target=e2e')) {
                return Promise.resolve('project1-e2e, project2-e2e');
            }
            else {
    ...
    it('should get e2e workflow for branch', async () => {
        ...
        const getCommandOutput = jest
            ...
            if (args.includes('--target=build-storybook')) {
                ...
                return Promise.resolve('project1-e2e, project2-e2e, project3-e2e');
            }
            else {
        ...
        expect(getCommandOutput).toHaveBeenCalled();
        expect(getCommandOutput.mock.lastCall[1]).toEqual([
            'nx',
            'print-affected',
