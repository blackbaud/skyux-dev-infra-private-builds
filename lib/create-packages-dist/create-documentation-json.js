...
async function findDependencies(codeExamples) {
    const dependencies = {};
    ...
    allDemoComponents.forEach((demoComponent) => {
        ...
        const demoComponentImports = demoComponentTsFiles
        ...
        demoComponentImports.forEach((dependency) => {
            ...
            else if (dependency.startsWith('@skyux/')) {
...
async function createDocumentationJson(projectName, distPackage) {
    ...
    let typedocCommand = 'node';
    let typedocPath;
    const typedocLocations = [
        (0, path_1.join)(CWD, 'node_modules/.bin/typedoc'),
        (0, path_1.join)(CWD, 'node_modules/@skyux/dev-infra-private/node_modules/.bin/typedoc'),
    ];
    for (const location of typedocLocations) {
        if ((0, fs_extra_1.existsSync)(location)) {
            typedocPath = [location];
            break;
        }
    }
    if (!typedocPath) {
    ...
    await (0, spawn_1.runCommand)(typedocCommand, [
        ...typedocPath,
    ...
    documentationJson.codeExampleDependencies = await findDependencies(documentationJson.codeExamples);
