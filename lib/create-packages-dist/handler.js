 ...
 */
function replacePlaceholderTextWithVersion(filePath, skyuxVersion, offsetPackageVersions) {
    const originalContents = (0, fs_extra_1.readFileSync)(filePath, { encoding: 'utf-8' });
    ...
    if (modifiedContents !== originalContents) {
        console.log(`Replaced "0.0.0-PLACEHOLDER" with "${skyuxVersion}" in file '${filePath}'.`);
        (0, fs_extra_1.writeFileSync)(filePath, modifiedContents);
    }
}
async function createPackagesDist(argv) {
    ...
    if (argv.projects) {
        ...
        for (const project of Object.values(distPackages)) {
            (0, fs_extra_1.emptyDirSync)(project.distRoot);
        }
    }
    else {
        (0, fs_extra_1.emptyDirSync)((0, path_1.join)(CWD, 'dist'));
    }
    // Build all libraries.
    ...
    for (const projectName in distPackages) {
        ...
        for (const versionedFilePath of versionedFilePaths) {
            if ((0, fs_extra_1.existsSync)((0, path_1.normalize)(versionedFilePath))) {
                replacePlaceholderTextWithVersion((0, path_1.normalize)(versionedFilePath), skyuxVersion, argv.offsetPackageVersions);
            }
        }
    }
    if (argv.projects) {
