...
async function createPackagesDist(argv) {
    ...
    for (const projectName in distPackages) {
        ...
        const versionedFilePaths = [
            (0, path_1.join)(distRoot, 'package.json'),
            (0, path_1.join)(distRoot, 'src/version.ts'),
            (0, path_1.join)(distRoot, 'src/schematics/migrations/migration-collection.json'),
        ];
        for (const versionedFilePath of versionedFilePaths) {
            if ((0, fs_extra_1.existsSync)(versionedFilePath)) {
                await replacePlaceholderTextWithVersion(versionedFilePath, skyuxVersion, skyuxPackagesVersion);
            }
        }
