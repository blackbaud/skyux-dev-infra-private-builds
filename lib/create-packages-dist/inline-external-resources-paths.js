...
exports.inlineExternalResourcesPaths = void 0;
 ...
 */
function inlineExternalResource(contents, externalResourceRegex, inlineResourceRegex) {
    ...
    componentDefinitionMatches?.forEach((componentDefinitionMatch) => {
        const externalResourceMatch = componentDefinitionMatch.match(externalResourceRegex);
        /*istanbul ignore else*/
        if (externalResourceMatch) {
            const externalResourcePath = externalResourceMatch[0];
            const inlineResourceMatch = componentDefinitionMatch.match(inlineResourceRegex);
            /*istanbul ignore else*/
    ...
    if (contents.match(externalResourceRegex)) {
        throw new Error('Relative file paths pointing to external resources were found in a component definition (e.g. `templateUrl` or `styleUrls`). ' +
...
function inlineTemplateUrls(contents) {
...
function inlineStyleUrls(contents) {
 ...
 */
function inlineExternalResourcesPaths(distPath) {
    console.log(`Inlining external resources for ${distPath}...`);
    const bundlePattern = (0, path_1.join)(distPath, '/bundles/*.umd.js');
    const bundlePaths = glob_1.default.sync(bundlePattern);
    if (bundlePaths.length > 0) {
        bundlePaths.forEach((bundlePath) => {
            let contents = (0, fs_extra_1.readFileSync)(bundlePath).toString();
            contents = inlineTemplateUrls(contents);
            contents = inlineStyleUrls(contents);
            (0, fs_extra_1.writeFileSync)(bundlePath, contents, { encoding: 'utf-8' });
            /*istanbul ignore next*/
            if ((0, fs_extra_1.existsSync)(`${bundlePath}.map`)) {
                (0, fs_extra_1.removeSync)(`${bundlePath}.map`);
            }
        });
        console.log(` ✔ Done inlining external resources for ${distPath}.`);
    }
    else {
        console.warn(`Skipping resource inlining. The UMD bundle was not found. (wanted '${bundlePattern}')`);
    }
}
exports.inlineExternalResourcesPaths = inlineExternalResourcesPaths;
